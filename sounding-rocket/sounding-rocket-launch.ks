...
// Functions
//  Format a time string as dd:HH:mm:SS
function toTime {
    parameter number.
...
function toSi {
    ...
    parameter number, places is 2.
    // si prefixes
    local prefixes is list("n", "μ", "m", "", "K", "M", "G", "T", "P", "E", "Z", "Y").

    // determine the largest power of 1000 smaller than our number
    local power is choose min(8, max(-3, floor(log10(abs(number)) / 3))) if number else 0.
